...
class Cluster:
    ...
    def avgDistance(self):
        ...
        if not self.points:
            return 0.0
        total = sum(p.distFrom(self.center) for p in self.points)
    ...
    def updateCenter(self):
        ...
        if not self.points:
            return self.center.coords
        coord_lists = [p.coords for p in self.points]
        avg_coords= [sum(dim_vals)/ len(dim_vals) for dim_vals in zip(*coord_lists)]
